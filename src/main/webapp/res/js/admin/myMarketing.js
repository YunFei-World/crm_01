layui.use(['table', 'jquery', 'form', 'laydate', 'upload'], function () {
    ...
    var type = 0;// 操作类型 0新增地点 1查看修改  市场营销活动管理
    var opID = '';// 需要操作的 数据id
    var key = '';
    ...
    var marketingTable = table.render({
        elem: '#marketing',
        url: 'marketing/findMyMarketingList?staff_id='+staff_id,
        toolbar: '#toolBar',
        title: '市场营销活动',
        cols: [[{
            title: '编号',
    ...
    var option_first = upload.render({
        ...
        , choose: function (obj) {
            ...
            obj.preview(function (index, file, result) {
                $('#option_first_all').html('<i class="layui-icon">&#xe605</i><p >已选择文件：' + file.name + "</p>");
            });
        }
        , done: function (res) {
        }
    ...
    var option_second = upload.render({
        ...
        , choose: function (obj) {
            obj.preview(function (index, file, result) {
                two = 'yes'
                $('#option_second_all').html('<i class="layui-icon">&#xe605</i><p >已选择文件：' + file.name + "</p>");
            });
        }
        , done: function (res) {
        }
    ...
    table.on('tool(marketing)', function (obj) {
        ...
        if (obj.event === 'edit') {
            ...
            if (filePath1 != "") {
                $('#option_first_all').html('<i class="layui-icon">&#xe605</i><p >已上传文件，可下载' + "</p>");
                $("#option-first-show").show();
            }
            if (filePath2 != "") {
                $('#option_second_all').html('<i class="layui-icon">&#xe605</i><p >已上传文件，可下载' + "</p>");
    ...
    function searchAdd() {
        ...
        table.reload('marketing', {
            url: 'marketing/findMyMarketingList?key=' + key + '&filter=' + Array.from(filter)+'&staff_id=' + staff_id
        });
    }
    ...
    $('#add-marketing').on('click', function () {
        ...
        $('#remark_first').val("");
        $('#remark_second').val("");
        $('#option_first_all').html('<i class="layui-icon"></i><p>点击上传，或将文件拖拽到此处</p>');
        $('#option_second_all').html('<i class="layui-icon"></i><p>点击上传，或将文件拖拽到此处</p>');
        $('#add-change').html('添加');
